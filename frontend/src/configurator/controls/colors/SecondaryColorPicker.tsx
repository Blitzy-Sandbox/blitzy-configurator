 ...
 *   - tickets/stories/ST-007-secondary-color-swatch-picker.md acceptance
 *     criteria (verbatim):
 *       AC1. The secondary-color palette is visible in the control sidebar
 *            and displays every swatch in the curated set.
 *       AC2. Clicking a swatch updates the preview's secondary panel color
 ...
 *       AC4. The secondary-color picker is reachable and operable using
 *            only keyboard input, and assistive technology announces each
 *            swatch's purpose and current selection state.
 *   - tickets/stories/ST-009-real-time-color-preview-sync.md AC2 — a
 *     secondary-color selection must reflect on the preview within the
 ...
 *   selectors and dispatches the `setSecondaryColor` action on user
 ...
 *   This component intentionally mirrors `AccentColorPicker.tsx` —
 *   inline curated palette, inline styles, schema-compliant imports
 *   from react and the configurator store only. The shape is also
 *   parallel to the SHIPPED `PrimaryColorPicker.tsx` DOM contract
 *   (same testid pattern, same aria-label template, same radiogroup
 *   structure) so the three pickers form a unified UI group.
 *
 ...
 *     - <ul role="radiogroup"> with `aria-label="Secondary panel color
 *       swatches"` so it is unambiguously addressable even though the
 *       page contains other radiogroups (primary picker, accent picker,
 *       stitching pattern, finish selector).
 *     - Each swatch is a native <button> with `role="radio"`,
 *       `aria-checked`, and an `aria-label` that announces both the
 *       swatch's purpose ("Secondary color <Name>") and its current
 ...
 *             This satisfies ST-007-AC4's "reachable AND operable
 ...
 *             focus AND dispatches `setSecondaryColor`, so screen-reader
 ...
 *       stylesheets or a high-contrast OS theme (ST-007-AC3).
 *
 ...
 *     - <section data-testid="secondary-color-picker"
 *                aria-label="Secondary panel color">
 *     - <ul role="radiogroup"
 *           aria-label="Secondary panel color swatches">
 *     - <button role="radio"
 *               aria-checked={isSelected}
 *               aria-label="Secondary color <Name> [(selected)]"
 ...
 *   The 8-entry palette below mirrors the test's `SECONDARY_PALETTE`
...
// ---------------------------------------------------------------------------
// Curated secondary palette (ST-007-AC1)
// ---------------------------------------------------------------------------

/**
 * One entry in the curated secondary palette. `value` is the canonical
 ...
 */
interface SecondarySwatch {
  readonly value: HexColor;
  readonly label: string;
}

/**
 * Curated secondary panel color palette per ST-007-AC1. Eight entries;
 * the FIRST entry is the documented store default
 * (`CONFIGURATOR_DEFAULTS.secondaryColor === '#000000'`) so the default
 * selected swatch is always present in the rendered set on first
 * render. The palette and ordering mirror the shipped
 * `SECONDARY_COLOR_SWATCHES` set referenced by the test suite
 ...
 *   - `#000000` Black             — default; classic two-tone partner
 *                                    for a white primary panel.
 *   - `#424242` Slate             — softened dark neutral.
 *   - `#9E9E9E` Grey              — mid-tone neutral.
 *   - `#FFFFFF` White             — high-contrast pairing for dark
 *                                    primary panels.
 *   - `#0D47A1` Deep blue         — saturated cool secondary.
 *   - `#1B5E20` Pine green        — saturated cool-warm secondary.
 *   - `#4101DB` Brand deep purple — Blitzy brand-aligned secondary.
 *   - `#B71C1C` Crimson           — saturated warm secondary.
 */
const SECONDARY_PALETTE: readonly SecondarySwatch[] = [
  { value: '#000000', label: 'Black' },
  { value: '#424242', label: 'Slate' },
  { value: '#9E9E9E', label: 'Grey' },
  { value: '#FFFFFF', label: 'White' },
  { value: '#0D47A1', label: 'Deep blue' },
  { value: '#1B5E20', label: 'Pine green' },
  { value: '#4101DB', label: 'Brand deep purple' },
  { value: '#B71C1C', label: 'Crimson' },
];

/**
 * Look up the friendly label for the currently-selected secondary color.
 *
 * The `loadDesign` action on the configurator store can hydrate
 * `secondaryColor` from a saved design payload that may carry a hex
 ...
 * "undefined".
 */
function findSecondaryLabel(hex: HexColor): string {
  const normalized = hex.toUpperCase();
  const match = SECONDARY_PALETTE.find((swatch) => swatch.value.toUpperCase() === normalized);
...
const CHECKMARK_STYLE: CSSProperties = {
  ...
  // (#FFFFFF, #9E9E9E) and dark (#000000, #4101DB) swatches without
 ...
 *      (ST-007-AC3). Both are applied to the SELECTED state so the
...
/**
 * The secondary panel color picker.
 *
 * Subscribes to the configurator store via slice-only selectors so a
 * primary-color or accent-color update does not trigger a re-render
 ...
 * sibling primary and accent pickers.
 *
 ...
 */
export function SecondaryColorPicker(): JSX.Element {
  // ----- Store subscriptions ------------------------------------------------
  // Slice-only selectors per Zustand best practice. `setSecondaryColor`
  ...
  // map over `SECONDARY_PALETTE` populates each slot synchronously on
  // mount.
  ...
  // ----- Derived values -----------------------------------------------------
  const currentLabel = findSecondaryLabel(secondaryColor);
   ...
   * documented latency budget (ST-007-AC2 / ST-009-AC2).
   */
   ...
   * preserved — the `pressing Space on a focused secondary swatch
   * activates it` test in `color-picker.spec.ts` exercises this
   * fall-through path.
   *
   * "Selection follows focus": the arrow key dispatches
   * `setSecondaryColor` so the selection AND the focus move together.
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent<HTMLButtonElement>, currentIndex: number) => {
      const len = SECONDARY_PALETTE.length;
      ...
      const nextSwatch = SECONDARY_PALETTE[nextIndex];
      ...
      setSecondaryColor(nextSwatch.value);
  ...
  return (
    <section
      style={SECTION_STYLE}
      // (CRITICAL — ST-007-AC4 assistive-technology contract)
    ...
    >
      <h3 style={HEADING_STYLE}>Secondary color</h3>
      <p style={HINT_STYLE}>
      ...
      <ul
        ...
        style={LIST_STYLE}
      >
        {SECONDARY_PALETTE.map((swatch, index) => {
          // Case-insensitive comparison so a saved design loaded in
          // mixed case (e.g., '#000000' lower-case from a legacy
