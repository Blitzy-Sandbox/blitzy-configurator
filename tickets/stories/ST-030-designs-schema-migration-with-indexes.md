...
- [ ] A forward migration introduces a designs table whose columns represent the server-assigned identifier, owning user reference, title, full design payload (colors, pattern, finish, logo reference and placement), and created/last-modified timestamps.
...
- [ ] A reverse migration is provided that returns the schema to its previous state, and both directions are idempotent against repeat application on a clean state.
