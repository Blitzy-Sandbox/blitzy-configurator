...
- [ ] The deploy stage is triggered after the build stage publishes an `IMAGE_DIGEST` for the same `COMMIT_SHA`, and accepts a `TARGET_ENV` input whose permitted values are the documented environment tokens (for example `development`, `staging`, `production`).
